...
from pybullet_envs_gymnasium.robot_manipulators import Pusher, Reacher, Striker, Thrower
...
class ReacherBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        electricity_cost = -0.10 * (
            np.abs(a[0] * self.robot.theta_dot) + np.abs(a[1] * self.robot.gamma_dot)
        ) - 0.01 * (  # work torque*angular_velocity
...
class PusherBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        electricity_cost = (
            -0.10 * action_product - 0.01 * action_sum  # work torque*angular_velocity  # stall torque require some energy
        )
...
class ThrowerBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        if (
            not self.robot._object_hit_ground and self.robot.object.pose().xyz()[2] < -0.25
        ):  # TODO: Should the object and target really belong to the robot? Maybe split this off
